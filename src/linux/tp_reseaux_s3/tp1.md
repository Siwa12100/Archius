...
#### OSPF 
**Open shortest path first** est un protocole de routage dynamique.
 Les protocoles de routage dynamique sont conçus pour **permettre aux routeurs de découvrir automatiquement les chemins les plus courts** vers les réseaux de destination. OSPF remplit cette fonction en échangeant des informations de routage avec d'autres routeurs au sein du même système autonome (AS). Ces informations de routage incluent des détails sur les liaisons entre les routeurs, les coûts associés à ces liaisons et d'autres métriques pour calculer les meilleures routes.
...
*A vérifier par pas eu le temps de l'expérimenter en tp, mais :*
Pour remplir ce fichier, il faut donc marquer pour chaque interface réseau : 
```
interface eth...
ip ospf hello-interval 5
ip ospf dead-interval 20
```
En gros, **chaque 5 secondes**, le routeur enverra **par le biais de cette interface un message hello** à destination des autres routeurs sur le réseau où cette interface est plongée, de manière à **dire qu'il est toujours actif.** 

Et dans le même sens, **si sur cette interface il ne reçoit pas de message hello dans une intervalle de 20 secondes**, c'est qu'**il considère que son voisin est mort**. Enfin plutôt que l'un de ses voisins est mort (celui qui n'a pas envoyé Hello), car un routeur peut avoir plusieurs voisins sur une même interface réseau. 

Il est donc important de définir cela pour toutes les interfaces réseau du routeur. 


Ensuite, à la fin du fichier, il faut rajouter :
```
router ospf
ospf router-id 1.1.1.4
network [ip d'un réseau où le routeur à un pied] area 0
...même ligne pour chaque réseau en lien direct avec le routeur...
```

Où **ospf router-id ... est l'identificateur unique du routeur OSPF**. Il doit donc falloir le changer pour chaque nouveau routeur dans le réseau. 
`area 0` fait référence à **la zone du réseau**, mais comme dit précédemment, dans le cadre du tp, il n'y a que la zone unique 0..

**Important :** Pour prendre en charge ces modifications, il faut ensuite faire la commande `systemctl restart ospdfd`. 

Pour afficher la base de données du démon OSPF, il suffit de s'y connecter par telnet (`telnet localhost 2604`) et de rentrer la commande 

Comme précédemment, pour connaitre dorénavant la route empruntée par les paquets pour aller de s1 à s2, on peut faire un `traceroute` depuis s1. 